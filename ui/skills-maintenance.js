const SkillsMaintenance = (() => {
  ...
  function renderReview(groups) {
    ...
    const rows = reviewGroups.map((group, index) => {
      ...
      const options = group.skills.map(id => {
  ...
  function selectedReviewChoices() {
    return reviewGroups.map((group, index) => {
  ...
  function markApplied(index) {
    ...
    container.querySelectorAll('input, button').forEach(input => { input.disabled = true; });
  ...
  async function runTidy() {
    ...
    if (!actionable) {
      addResult('Library tidy', summary.reviewNeeded ? `${summary.reviewNeeded} item(s) need manual review` : 'Nothing to move or remove');
      return;
    }
    ...
    addResult('Library tidy', `${done.moved || 0} moved, ${done.duplicatesRemoved || 0} duplicates removed, ${done.emptyDirsRemoved || 0} empty folders removed`, 'ok');
  }
  ...
  async function runReview() {
    ...
    addResult('Similarity review', groups.length ? `${groups.length} possible overlap group(s) flagged` : 'No likely overlaps found', groups.length ? 'warn' : 'ok');
    renderReview(groups);
  }
