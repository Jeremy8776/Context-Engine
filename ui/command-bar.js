  ...
  const CMDS = [
    {
      id: 'dashboard',
      label: 'Go to Context',
      keys: ['context', 'dashboard', 'overview', 'home'],
      run: () => switchTabById('dashboard'),
    },
    { id: 'skills', label: 'Go to Skills', keys: ['skills', 'library'], run: () => switchTabById('skills') },
    {
      id: 'modes',
      label: 'Go to Modes & Workflows',
      keys: ['modes', 'presets', 'workflows', 'chains', 'runs'],
      run: () => switchTabById('modes'),
    },
    { id: 'memory', label: 'Go to Memory', keys: ['memory', 'about me'], run: () => switchTabById('memory') },
    {
      id: 'config',
      label: 'Go to Rules',
      keys: ['rules', 'soul', 'config', 'keys'],
      run: () => switchTabById('config'),
    },
    {
      id: 'compile',
      label: 'Go to Outputs',
    ...
    },
    {
      ...
      keys: ['discover', 'scan'],
      run: () => typeof DashboardTab !== 'undefined' && DashboardTab.discover(),
    },
    {
      id: 'regen',
      label: 'Regenerate CONTEXT.md',
      keys: ['regen', 'context', 'refresh'],
      run: () => typeof DashboardTab !== 'undefined' && DashboardTab.regenCONTEXTmd(),
    },
    {
      id: 'backup',
      label: 'Create Backup',
      keys: ['backup', 'snapshot', 'save state'],
      run: () => typeof DashboardTab !== 'undefined' && DashboardTab.backup(),
    },
