  ...
  return {
    ...
    endpoints: [
      ...
      },
      {
        ...
        description: 'Clone a skill repo into skills/ingested (allowlisted hosts only)',
      },
      { method: 'GET', path: '/api/skills/ingest/:jobId', description: 'Poll an in-flight ingest job' },
      {
        method: 'POST',
        path: '/api/skills/parse',
        description: 'LLM-parse skill descriptions for unparsed entries',
      },
      {
        method: 'POST',
        path: '/api/skills/organise',
        description: 'Tidy skill library (move/remove/review)',
      },
      ...
      // Skill sources (Phase 1 + 2 — Link + Import + Sync)
      {
        ...
        description: 'List registered skill sources + implicit internal',
      },
      { method: 'POST', path: '/api/skill-sources', description: 'Link an external skill directory' },
      {
        method: 'DELETE',
        path: '/api/skill-sources/:id',
        description: 'Unlink a source (manifest dropped; imported tree kept)',
      },
      { method: 'GET', path: '/api/skill-sources/scan', description: 'Probe known host-app skill paths' },
      {
        method: 'POST',
        path: '/api/skill-sources/:id/import',
        description: 'Hard-link or copy a source into <CE_ROOT>/skills/imported/<id>/',
      },
      {
        method: 'GET',
        path: '/api/skill-sources/:id/sync',
        description: 'Diff source vs imported tree (added/removed/modified/localEdits/conflicts)',
      },
      {
        method: 'POST',
        path: '/api/skill-sources/:id/sync/apply',
        description: 'Apply sync diff. Body { mode: "append" | "overwrite" }',
      },
      ...
      ...extra,
      {
        method: 'GET',
        path: '/api/onboarding',
        description: 'First-run discovery summary (hosts + tools + context + index)',
      },
      {
        method: 'POST',
        path: '/api/onboarding/complete',
        description: 'Mark onboarding complete (suppresses re-prompt)',
      },
      {
        method: 'POST',
        path: '/api/onboarding/reset',
        description: 'Re-arm the onboarding flow for the next launch',
      },
