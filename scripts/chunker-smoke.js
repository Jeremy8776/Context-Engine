...
assert(chunks.some(chunk => chunk.type === 'rule'), 'expected a rule chunk');
assert(chunks.some(chunk => chunk.type === 'example'), 'expected an example chunk');
assert(chunks.every(chunk => chunk.skillId === 'fixture-skill'), 'expected skill id on every chunk');
assert(chunks.every(chunk => chunk.sourcePath.endsWith('SKILL.md')), 'expected source path on every chunk');
...
assert(complexChunks.some(chunk => chunk.section === 'Discovery'), 'expected nested discovery section');
assert(complexChunks.some(chunk => chunk.section === 'Execution'), 'expected nested execution section');
assert(complexChunks.some(chunk => chunk.type === 'example'), 'expected complex example chunk');
